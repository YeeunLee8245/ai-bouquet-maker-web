...
import { formatBouquetRecipeDetail } from '../helper';
...
export async function GET(
  ...
  try {
    ...
    const isOwner = publicUser ? publicUser.id === bouquet.user_id : false;

    // 4. 서명 검증 및 카드 메시지 노출 가공
    ...
    if (!isOwner && !signatureValid) {
      // 본인이 아니고 유효한 서명도 없는 경우 메시지를 숨깁니다.
      message = null;
    }

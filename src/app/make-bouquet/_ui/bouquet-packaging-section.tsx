...
import { bouquetPackagingColorAtom, bouquetRibbonColorAtom } from '../_model/bouquet-form.atoms';
    ...
    <div>
      ...
      <div className='flex flex-wrap gap-1 mt-2'>
        {colors.map((color: string, index: number) => (
          <ColorPicker key={color} color={color} ref={(el) => { colorRefs.current[index] = el; }} onClick={() => handleSelectColor(index)} />
        ))}
        {/* 색상 추가 버튼 */}
        <ColorPicker
          data-state='default'

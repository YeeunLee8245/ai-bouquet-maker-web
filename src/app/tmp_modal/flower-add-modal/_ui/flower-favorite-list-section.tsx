...
function FlowerFavoriteListSection() {
  return (
    <section>
      <SearchInput placeholder='꽃 이름, 꽃말, 설명 등으로 검색' className='w-full h-[40px]'/>
      {TEMP_FLOWER_SEARCH_RESULT.length > 0 && (
        <div className='mt-2 rounded-5 border-1 border-gray-100 max-h-[172px] overflow-y-scroll'>
          {TEMP_FLOWER_SEARCH_RESULT.map(({ id, name, colors }) => (
            <div key={id} className='px-4 py-3 flex justify-between items-center'>
              <div>
...
export default FlowerFavoriteListSection;
